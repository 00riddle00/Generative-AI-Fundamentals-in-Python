...
max_tokens_per_message = st.sidebar.slider(
    "Max Tokens Per Message", min_value=10, max_value=500, value=50
)
...
system_message = st.sidebar.selectbox(
    "System message", ["Sassy", "Angry", "Thoughtful", "Custom"]
)
...
if st.sidebar.button(
    "Reset conversation history", on_click=chat_manager.reset_conversation_history
):
...
if user_input:
    response = chat_manager.chat_completion(
        user_input, temperature=temperature, max_tokens=max_tokens_per_message
    )
